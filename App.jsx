...
function FinancialHealthDashboard({ totalMonthlyIncome, bills, totalMonthlySavings, totalCDValue, totalCDPrincipal, scenarios, buckets, icons }) {
  const n = (v) => parseFloat(v) || 0;
  const totalBillsFixed = n(bills.rent) + n(bills.carPayment)
    + bills.insurances.reduce((s, i) => s + n(i.amount), 0)
    + bills.others.reduce((s, o) => s + n(o.amount), 0);
  const remaining = Math.max(0, totalMonthlyIncome - totalBillsFixed - totalMonthlySavings);
  const committedRatio = totalMonthlyIncome > 0 ? Math.round(((totalBillsFixed + totalMonthlySavings) / totalMonthlyIncome) * 100) : 0;
  const savingsRatio = totalMonthlyIncome > 0 ? Math.round((totalMonthlySavings / totalMonthlyIncome) * 100) : 0;
  const recessionScenario = [...scenarios].reverse().find(s => s.type === "recession");
  const runway = recessionScenario?.data?.runway;
  ...
  const healthTier =
    healthScore >= 75 ? { label: "Strong", color: C.green, desc: "Your finances are in great shape." } :
  ...
  const donutData = [
    { label: "Bills", value: totalBillsFixed, color: "#e07b3a" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* HEALTH SCORE HERO */}
      <Card style={{ background: C.bg === "#0a0f1e" ? `linear-gradient(135deg, ${C.card} 0%, #1a2f52 100%)` : `linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%)`, border: `1.5px solid ${healthTier.color}33` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ position: "relative", width: 100, height: 100, flexShrink: 0 }}>
            <svg width="100" height="100" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="42" fill="none" stroke={C.border} strokeWidth="9" />
              <circle cx="50" cy="50" r="42" fill="none" stroke={healthTier.color} strokeWidth="9"
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontSize: 26, fontWeight: 700, color: healthTier.color, lineHeight: 1 }}>{healthScore}</div>
              <div style={{ fontSize: 8, color: C.muted, textTransform: "uppercase", letterSpacing: "0.08em", marginTop: 2 }}>Health</div>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 20, color: healthTier.color, marginBottom: 4 }}>{healthTier.label}</div>
            <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>{healthTier.desc}</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <div style={{ background: `${healthTier.color}15`, borderRadius: 8, padding: "5px 10px", fontSize: 11, color: healthTier.color, fontWeight: 700 }}>{committedRatio}% committed</div>
              <div style={{ background: `${C.green}15`, borderRadius: 8, padding: "5px 10px", fontSize: 11, color: C.green, fontWeight: 700 }}>{savingsRatio}% saving</div>
            </div>
          </div>
        </div>
      </Card>
      ...
      {totalBillsFixed > 0 && (
        <Card>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Monthly Bills Breakdown</div>
          {[
            ["🏠 Rent / Mortgage", n(bills.rent), "#0284c7"],
            ["🚗 Car Payment", n(bills.carPayment), "#d97706"],
            ...
            <div key={label} style={{ marginBottom: 10 }}>
              ...
              <div style={{ height: 6, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(100, (val / totalBillsFixed) * 100)}%`, background: color, borderRadius: 3, transition: "width 0.6s ease" }} />
              </div>
            </div>
          ))}
          <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, borderTop: `1px solid ${C.border}`, marginTop: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: C.text }}>Total Bills</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#e07b3a" }}>{fmt(totalBillsFixed)}/mo · {fmt(totalBillsFixed * 12)}/yr</span>
          </div>
        </Card>
      )}
        ...
        <Card>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {scenarios.map(s => {
              ...
              const sc = score >= 75 ? C.green : score >= 50 ? "#d97706" : score >= 25 ? "#e07b3a" : C.red;
              return (
                <div key={s.id} style={{ background: C.bg, border: `1px solid ${sc}33`, borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: sc }}>{score}</div>
      ...
      {/* EMPTY STATE */}
      {totalMonthlyIncome === 0 && totalBillsFixed === 0 && scenarios.length === 0 && (
        <Card style={{ textAlign: "center", padding: 32 }}>
          ...
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7 }}>
            Add your income in the <strong>Income</strong> tab, bills in the <strong>Bills</strong> tab, and run a simulator to start building your financial picture.
          </div>
        </Card>
      )}
...
function Dashboard({ user, onLogout, onShowAuth }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {activeTab === "overview" && <FinancialHealthDashboard totalMonthlyIncome={totalMonthlyIncome} bills={bills} totalMonthlySavings={totalMonthlySavings} totalCDValue={totalCDValue} totalCDPrincipal={totalCDPrincipal} scenarios={scenarios} buckets={buckets} icons={icons} />}
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <Card>
              ...
              {bills.insurances.map((ins, idx) => (
                <div key={ins.id} style={{ display: "flex", gap: 8, alignItems: "flex-end", marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Type</div>
                    <input value={ins.label} onChange={e => { const updated = bills.insurances.map(x => x.id === ins.id ? { ...x, label: e.target.value } : x); saveBills({ ...bills, insurances: updated }); }}
                  ...
                  <div style={{ width: 110 }}>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input type="number" value={ins.amount} onChange={e => { const updated = bills.insurances.map(x => x.id === ins.id ? { ...x, amount: e.target.value } : x); saveBills({ ...bills, insurances: updated }); }}
                        placeholder="0"
              ...
              {bills.others.map((other) => (
                <div key={other.id} style={{ display: "flex", gap: 8, alignItems: "flex-end", marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input value={other.label} onChange={e => { const updated = bills.others.map(x => x.id === other.id ? { ...x, label: e.target.value } : x); saveBills({ ...bills, others: updated }); }}
                  ...
                  <div style={{ width: 110 }}>
                    ...
                    <div style={{ position: "relative" }}>
                      <span style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 13 }}>$</span>
                      <input type="number" value={other.amount} onChange={e => { const updated = bills.others.map(x => x.id === other.id ? { ...x, amount: e.target.value } : x); saveBills({ ...bills, others: updated }); }}
                        placeholder="0"
                        style={{ width: "100%", padding: "10px 10px 10px 20px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                    </div>
                  </div>
