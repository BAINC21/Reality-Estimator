...
function SavingsBucket({ bucket, onUpdate, onDelete }) {
  ...
  const handleAdjust = () => {
    ...
    setAdjustAmt(""); setShowAdjust(false);
  };
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                ...
                {monthsLeft === 0 ? <span style={{ color: C.green }}> · 🎉 Goal reached!</span> : monthsLeft ? <span> · ~{monthsLeft} mo to go</span> : null}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button onClick={() => { setShowAdjust(v => !v); setAdjustAmt(""); setAdjustMode("add"); }}
                style={{ width: 28, height: 28, borderRadius: 8, background: showAdjust ? color : `${color}22`, border: `1.5px solid ${color}`, color: showAdjust ? "#fff" : color, fontSize: 18, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {showAdjust ? "×" : "+"}
              </button>
          ...
          {showAdjust && (
            <div style={{ background: C.bg, border: `1.5px solid ${color}44`, borderRadius: 12, padding: 14, marginTop: 4 }}>
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>{adjustMode === "add" ? "Amount to add ($)" : "Amount to withdraw ($)"}</div>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 14 }}>$</span>
                  <input type="number" value={adjustAmt} onChange={e => setAdjustAmt(e.target.value)} placeholder="0.00" autoFocus
                ...
                return (
                  <div style={{ background: `${color}11`, border: `1px solid ${color}33`, borderRadius: 8, padding: "8px 12px", marginBottom: 10, fontSize: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: C.muted }}>New balance</span><span style={{ fontWeight: 700, color }}>{fmt(newSaved)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: C.muted }}>Progress</span><span style={{ fontWeight: 700, color }}>{newPct}%</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: C.muted }}>New timeline</span>
                      <span style={{ fontWeight: 700, color: newMonths === 0 ? C.green : color }}>{newMonths === 0 ? "🎉 Goal reached!" : `~${newMonths} months`}</span>
                    </div>
                  </div>
                );
              })()}
              <Btn onClick={handleAdjust} style={{ width: "100%", padding: "11px", background: adjustMode === "add" ? C.green : C.red, fontSize: 13 }}>
...
function HealthDashboard({ totalMonthlyIncome, totalMonthlySavings, totalCDValue, totalCDPrincipal, bills, buckets, scenarios, icons, colors }) {
  ...
  const remaining = Math.max(0, totalMonthlyIncome - totalBills - totalMonthlySavings);
  ...
  const cdGrowth = totalCDValue - totalCDPrincipal;
  const healthScore = Math.max(0, Math.min(100, Math.round(
    (committedRatio <= 50 ? 30 : committedRatio <= 65 ? 18 : committedRatio <= 80 ? 8 : 0) +
    (savingsRatio >= 20 ? 25 : savingsRatio >= 10 ? 15 : savingsRatio >= 5 ? 8 : 0) +
    (runway >= 6 ? 20 : runway >= 3 ? 12 : runway >= 1 ? 5 : 10) +
    (totalCDPrincipal > 0 ? 10 : 0) +
    (remaining > 500 ? 15 : remaining > 200 ? 8 : 0)
  )));
  const tier = healthScore >= 75 ? { label: "Strong", color: C.green, desc: "Your finances are in great shape." }
    : healthScore >= 55 ? { label: "Stable", color: "#0284c7", desc: "Solid foundation with room to grow." }
    : healthScore >= 35 ? { label: "Building", color: "#d97706", desc: "Making progress — a few areas to strengthen." }
    : { label: "Needs Attention", color: "#e07b3a", desc: "Some areas could use focus right now." };
  const donutItems = [
    { label: "Bills", value: totalBills, color: "#e07b3a" },
    { label: "Savings", value: totalMonthlySavings, color: C.green },
    { label: "Free", value: remaining, color: C.primary },
  ].filter(d => d.value > 0);
  const donutTotal = donutItems.reduce((s, d) => s + d.value, 0) || 1;
  const R = 46, SW = 14, CX = 60, CY = 60, circ = 2 * Math.PI * R;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <Card style={{ background: C.bg === "#0a0f1e" ? `linear-gradient(135deg, ${C.card}, #1a2f52)` : "linear-gradient(135deg, #eff6ff, #f0fdf4)", border: `1.5px solid ${tier.color}33` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ position: "relative", width: 100, height: 100, flexShrink: 0 }}>
            <svg width="100" height="100" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="42" fill="none" stroke={C.border} strokeWidth="9" />
              <circle cx="50" cy="50" r="42" fill="none" stroke={tier.color} strokeWidth="9"
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontSize: 26, fontWeight: 700, color: tier.color, lineHeight: 1 }}>{healthScore}</div>
              <div style={{ fontSize: 8, color: C.muted, textTransform: "uppercase", letterSpacing: "0.08em", marginTop: 2 }}>Health</div>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 20, color: tier.color, marginBottom: 4 }}>{tier.label}</div>
            <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>{tier.desc}</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <div style={{ background: `${tier.color}18`, borderRadius: 8, padding: "5px 10px", fontSize: 11, color: tier.color, fontWeight: 700 }}>{committedRatio}% committed</div>
              <div style={{ background: `${C.green}18`, borderRadius: 8, padding: "5px 10px", fontSize: 11, color: C.green, fontWeight: 700 }}>{savingsRatio}% saving</div>
            </div>
          </div>
        </div>
      </Card>
      {totalMonthlyIncome > 0 && (
        <Card>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Income Allocation</div>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <svg width="120" height="120" viewBox="0 0 120 120" style={{ flexShrink: 0 }}>
              {donutItems.map((seg, idx) => {
                const prevPct = donutItems.slice(0, idx).reduce((s, d) => s + d.value / donutTotal, 0);
                const pct = seg.value / donutTotal;
                return (
                  <circle key={seg.label} cx={CX} cy={CY} r={R} fill="none"
                    stroke={seg.color} strokeWidth={SW}
                    strokeDasharray={`${circ * pct} ${circ * (1 - pct)}`}
                    strokeDashoffset={circ * (1 - prevPct)}
                    transform={`rotate(-90 ${CX} ${CY})`} />
                );
              })}
              <text x={CX} y={CY - 5} textAnchor="middle" fill={C.text} fontSize="13" fontWeight="700">{fmt(totalMonthlyIncome)}</text>
              <text x={CX} y={CY + 10} textAnchor="middle" fill={C.muted} fontSize="9">/month</text>
            </svg>
            <div style={{ flex: 1 }}>
              {donutItems.map(seg => (
                <div key={seg.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, color: C.text }}>{seg.label}</span>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: seg.color }}>{fmt(seg.value)}</div>
                    <div style={{ fontSize: 10, color: C.muted }}>{Math.round((seg.value / donutTotal) * 100)}%</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Card style={{ textAlign: "center", padding: "14px 12px" }}>
          <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Free Cash/mo</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: remaining > 500 ? C.green : remaining > 0 ? "#d97706" : C.red }}>{fmt(remaining)}</div>
          <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>after bills & savings</div>
        </Card>
        <Card style={{ textAlign: "center", padding: "14px 12px" }}>
          <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Saving This Year</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: C.green }}>{fmt(totalMonthlySavings * 12)}</div>
          <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>{fmt(totalMonthlySavings)}/mo committed</div>
        </Card>
        <Card style={{ textAlign: "center", padding: "14px 12px" }}>
          <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Crisis Runway</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: runway >= 6 ? C.green : runway >= 3 ? "#d97706" : runway > 0 ? "#e07b3a" : C.muted }}>
            {runway >= 99 ? "∞" : runway > 0 ? `${runway} mo` : "—"}
          </div>
          <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>{runway ? "from recession sim" : "run recession sim"}</div>
        </Card>
        <Card style={{ textAlign: "center", padding: "14px 12px" }}>
          <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>CD Growth</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: cdGrowth > 0 ? "#7c3aed" : C.muted }}>{cdGrowth > 0 ? `+${fmt(cdGrowth)}` : "—"}</div>
          <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>{totalCDPrincipal > 0 ? `${fmt(totalCDPrincipal)} invested` : "no CDs yet"}</div>
        </Card>
      </div>
      {totalBills > 0 && (
        <Card>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Monthly Bills</div>
          {[["🏠 Rent / Mortgage", n(bills.rent), "#0284c7"], ["🚗 Car Payment", n(bills.carPayment), "#d97706"],
            ["🛡️ Insurance", bills.insurances.reduce((s, i) => s + n(i.amount), 0), "#7c3aed"],
            ["📦 Other", bills.others.reduce((s, o) => s + n(o.amount), 0), "#6b7280"]
          ].filter(([, v]) => v > 0).map(([label, val, color]) => (
            <div key={label} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 12, color: C.text }}>{label}</span>
                <span style={{ fontSize: 12, fontWeight: 600, color }}>{fmt(val)}/mo</span>
              </div>
              <div style={{ height: 6, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(100, (val / totalBills) * 100)}%`, background: color, borderRadius: 3, transition: "width 0.6s ease" }} />
              </div>
            </div>
          ))}
          <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, borderTop: `1px solid ${C.border}`, marginTop: 2 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: C.text }}>Total</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#e07b3a" }}>{fmt(totalBills)}/mo · {fmt(totalBills * 12)}/yr</span>
          </div>
        </Card>
      )}
      {buckets.length > 0 && (
      ...
      {scenarios.length > 0 && (() => {
        const avgScore = Math.round(scenarios.reduce((s, sc) => s + (sc.data?.score || 0), 0) / scenarios.length);
        const avgTier = avgScore >= 75 ? { label: "Strong", color: C.green }
          : avgScore >= 55 ? { label: "Stable", color: "#0284c7" }
          : avgScore >= 35 ? { label: "Building", color: "#d97706" }
          : { label: "Needs Attention", color: "#e07b3a" };
        const n = (v) => parseFloat(v) || 0;
        const totalBillsForLabel = n(bills.rent) + n(bills.carPayment)
          + bills.insurances.reduce((s, i) => s + n(i.amount), 0)
          + bills.others.reduce((s, o) => s + n(o.amount), 0);
        const labelRows = [
          ...
          { icon: "📋", label: "Saved Scenarios", value: `${scenarios.length}`, color: C.muted },
          ...scenarios.map(s => ({
            icon: icons[s.type] || "📋",
            label: s.label,
          ...
          { icon: "🧾", label: "Total Bills", value: totalBillsForLabel > 0 ? fmt(totalBillsForLabel) + "/mo" : "—", color: "#e07b3a" },
              ...
              <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                <div style={{ position: "relative", width: 90, height: 90 }}>
                  <svg width="90" height="90" viewBox="0 0 90 90">
                    ...
                    <circle cx="45" cy="45" r="35" fill="none" stroke={avgTier.color} strokeWidth="8"
                      strokeDasharray={`${2 * Math.PI * 35} ${2 * Math.PI * 35}`}
                      strokeDashoffset={`${2 * Math.PI * 35 * (1 - avgScore / 100)}`}
                      strokeLinecap="round" transform="rotate(-90 45 45)"
                      style={{ transition: "stroke-dashoffset 0.8s ease" }} />
                  </svg>
                  <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: avgTier.color, lineHeight: 1 }}>{avgScore}</div>
                    <div style={{ fontSize: 8, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 2 }}>avg</div>
                  </div>
                </div>
                <div style={{ fontSize: 11, fontWeight: 700, color: avgTier.color }}>{avgTier.label}</div>
              </div>
              {/* Right — label rows */}
              <div style={{ flex: 1 }}>
                {labelRows.map((row, i) => (
      ...
      {totalMonthlyIncome === 0 && totalBills === 0 && scenarios.length === 0 && (
        <Card style={{ textAlign: "center", padding: 32 }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>📊</div>
          <div style={{ fontWeight: 700, fontSize: 16, color: C.text, marginBottom: 8 }}>Your health dashboard is empty</div>
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7 }}>Add your income in the <strong>Income</strong> tab, bills in the <strong>Bills</strong> tab, and run a simulator to start building your financial picture.</div>
        </Card>
      )}
    </div>
  );
...
function Dashboard({ user, onLogout, onShowAuth }) {
  ...
  const saveBills = (v) => { setBills(v); DB_DASH.set("bills", v); };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {activeTab === "overview" && (
        <HealthDashboard
          ...
          totalCDValue={totalCDValue}
          totalCDPrincipal={totalCDPrincipal}
          bills={bills}
          buckets={buckets}
          scenarios={scenarios}
          icons={icons}
      ...
      {activeTab === "bills" && (() => {
        ...
        const score = Math.max(0, Math.min(100, ratio <= 40 ? 100 - ratio : ratio <= 60 ? 60 - (ratio - 40) : Math.max(0, 40 - (ratio - 60) * 2)));
        const tier = score >= 60 ? { label: "Comfortable", color: C.green, bg: "rgba(15,118,110,0.08)", border: "rgba(15,118,110,0.25)" }
          : score >= 30 ? { label: "Manageable", color: "#d97706", bg: "rgba(217,119,6,0.08)", border: "rgba(217,119,6,0.25)" }
          : { label: "Tight", color: "#e07b3a", bg: "rgba(224,123,58,0.08)", border: "rgba(224,123,58,0.25)" };
        const BillInput = ({ label, value, onChange }) => (
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>{label}</div>
            <div style={{ position: "relative" }}>
              <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 14 }}>$</span>
              <input type="number" value={value} onChange={e => onChange(e.target.value)} placeholder="0"
                style={{ width: "100%", padding: "11px 12px 11px 26px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
            </div>
          </div>
        );
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <Card>
              ...
              {bills.insurances.map((ins) => (
                <div key={ins.id} style={{ display: "flex", gap: 8, alignItems: "flex-end", marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Type</div>
                    <input value={ins.label} onChange={e => saveBills({ ...bills, insurances: bills.insurances.map(x => x.id === ins.id ? { ...x, label: e.target.value } : x) })}
                      placeholder="e.g. Car Insurance"
                      style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                  </div>
                  <div style={{ width: 110 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>$/mo</div>
                    <div style={{ position: "relative" }}>
                      <span style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 13 }}>$</span>
                      <input type="number" value={ins.amount} onChange={e => saveBills({ ...bills, insurances: bills.insurances.map(x => x.id === ins.id ? { ...x, amount: e.target.value } : x) })}
                        placeholder="0"
                        style={{ width: "100%", padding: "10px 10px 10px 20px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                    </div>
                  </div>
                  {bills.insurances.length > 1 && (
                    <button onClick={() => saveBills({ ...bills, insurances: bills.insurances.filter(x => x.id !== ins.id) })}
                      style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 18, padding: "0 4px", paddingBottom: 10 }}>✕</button>
                  )}
                </div>
              ))}
              {totalInsurance > 0 && <div style={{ fontSize: 12, color: C.muted, borderTop: `1px solid ${C.border}`, paddingTop: 8, display: "flex", justifyContent: "space-between" }}><span>Total Insurance</span><span style={{ fontWeight: 600, color: C.text }}>{fmt(totalInsurance)}/mo · {fmt(totalInsurance * 12)}/yr</span></div>}
            </Card>
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              {bills.others.length === 0 && <div style={{ fontSize: 13, color: C.muted, textAlign: "center", padding: "12px 0" }}>Tap + to add subscriptions, utilities, etc.</div>}
              {bills.others.map((other) => (
                <div key={other.id} style={{ display: "flex", gap: 8, alignItems: "flex-end", marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Name</div>
                    <input value={other.label} onChange={e => saveBills({ ...bills, others: bills.others.map(x => x.id === other.id ? { ...x, label: e.target.value } : x) })}
                      placeholder="e.g. Netflix, Electric, Gym"
                      style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                  </div>
                  <div style={{ width: 110 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>$/mo</div>
                    <div style={{ position: "relative" }}>
                      <span style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 13 }}>$</span>
                      <input type="number" value={other.amount} onChange={e => saveBills({ ...bills, others: bills.others.map(x => x.id === other.id ? { ...x, amount: e.target.value } : x) })}
                        placeholder="0"
                        style={{ width: "100%", padding: "10px 10px 10px 20px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                    </div>
                  </div>
              ...
              {totalOther > 0 && <div style={{ fontSize: 12, color: C.muted, borderTop: `1px solid ${C.border}`, paddingTop: 8, display: "flex", justifyContent: "space-between" }}><span>Total Other</span><span style={{ fontWeight: 600, color: C.text }}>{fmt(totalOther)}/mo · {fmt(totalOther * 12)}/yr</span></div>}
            </Card>
            {totalBills > 0 && (
              <Card>
      ...
      {activeTab === "scenarios" && (() => {
        const ScenarioDetail = ({ s }) => {
          const d = s.data || {};
          const rows = s.type === "moving" ? [
            ["Monthly Income", fmt(d.income)], ["Rent", fmt(d.rent)], ["Utilities", fmt(d.utilities)],
            ["Groceries", fmt(d.groceries)], ["Insurance", fmt(d.insurance)], ["Subscriptions", fmt(d.subscriptions)],
            ["Furniture", fmt(d.furniture)], ["Parking", fmt(d.parking)], ["Savings Goal", fmt(d.savings)],
            ["Total Monthly", fmt(d.total), true], ["% of Income", `${d.ratio}%`, true], ["Surplus", fmt(d.surplus), true],
          ] : s.type === "car" ? [
            ["Monthly Income", fmt(d.income)], ["Car Payment", fmt(d.payment)], ["Insurance", fmt(d.insurance)],
            ["Gas", fmt(d.gas)], ["Maintenance", fmt(d.maintenance)],
            ["Monthly Total", fmt(d.total), true], ["% of Income", `${d.ratio}%`, true], ["Annual Cost", fmt(d.annualCost), true],
          ] : s.type === "project" ? [
            ["Materials", fmt(d.materials)], ["Labor", fmt(d.labor)], ["Time Cost", fmt(d.timeCost)],
            ["Hidden / Delivery", fmt(d.hidden)], ["Buffer Added", fmt(d.bufferAmt)], ["Reality Total", fmt(d.total), true],
          ] : s.type === "recession" ? [
            ["Monthly Income", fmt(d.income)], ["Current Savings", fmt(d.savings)], ["Essential Expenses", fmt(d.essentials)],
            ["Income Drop", `${d.drop}%`], ["Emergency Reserve", fmt(d.reserve)], ["Reduced Income", fmt(d.reduced)],
            ["Monthly Gap", fmt(d.gap)], ["Runway", d.runway >= 99 ? "Indefinite ✓" : `${d.runway} months`, true],
          ] : s.type === "debt" ? [
            ["Strategy", d.strategy === "avalanche" ? "Avalanche" : "Snowball"],
            ["Total Balance", fmt(d.totalBalance)], ["Monthly Payment", fmt(d.totalMonthly)],
            ["Payoff Time", `${d.payoffMonths} mo`], ["Total Interest", fmt(d.totalInterest)],
            ["Interest Saved", fmt(d.interestSaved), true],
          ] : [];
          const accent = colors[s.type] || C.primary;
          return (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {rows.map(([label, value, highlight]) => (
                  <div key={label} style={{ background: highlight ? `${accent}12` : C.bg, borderRadius: 8, padding: "8px 10px", border: highlight ? `1px solid ${accent}30` : `1px solid ${C.border}` }}>
                    <div style={{ fontSize: 10, color: C.muted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 2 }}>{label}</div>
                    <div style={{ fontSize: 14, fontWeight: highlight ? 700 : 500, color: highlight ? accent : C.text }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {activeTab === "settings" && (
        ...
        }} onLogout={onLogout} showEmail={showEmail} onToggleEmail={toggleShowEmail} />
      )}
