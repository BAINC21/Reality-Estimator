...
function HealthDashboard({ totalMonthlyIncome, totalMonthlySavings, totalCDValue, totalCDPrincipal, bills, buckets, scenarios, icons, colors }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {scenarios.length > 0 && (
        <Card>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Scenario Portfolio</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {scenarios.map(s => {
              const sc = (s.data?.score || 0) >= 75 ? C.green : (s.data?.score || 0) >= 50 ? "#d97706" : (s.data?.score || 0) >= 25 ? "#e07b3a" : C.red;
              return (
                <div key={s.id} style={{ background: C.bg, border: `1px solid ${sc}33`, borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      <div style={{ fontSize: 16, marginBottom: 3 }}>{icons[s.type] || "📋"}</div>
                    ...
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: sc }}>{s.data?.score || 0}</div>
          ...
          <div style={{ marginTop: 12, padding: "10px 12px", background: C.bg, borderRadius: 10, display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, color: C.muted }}>Avg scenario score</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{Math.round(scenarios.reduce((s, sc) => s + (sc.data?.score || 0), 0) / scenarios.length)}/100</span>
          </div>
        </Card>
      )}
