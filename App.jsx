...
const Input = ({ label, value, onChange, placeholder = "", type = "text", style = {} }) => (
  <div style={{ marginBottom: 14 }}>
    ...
    <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} type={type}
      style={{ width: "100%", padding: "12px 14px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 14, fontFamily: "inherit", outline: "none", color: C.text, background: "#f8fafc", boxSizing: "border-box", ...style }} />
  </div>
);
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        ...
        <input
          ...
          style={{
            ...
            fontFamily: "inherit", outline: "none", color: C.text, background: "#f8fafc",
  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: size * 0.22, fontWeight: 800, color: tier.color, lineHeight: 1 }}>{score}</div>
...
function CarOwnershipSim({ user, onSave }) {
  ...
  const handleSave = () => {
    if (!user) return alert("Create an account to save scenarios!");
    onSave({ id: Date.now().toString(), type: "car", label: `Car Ownership — ${fmt(total)}/mo`, date: new Date().toLocaleDateString(), data: { income, payment, insurance, gas, maintenance, total, ratio, annualCost, score } });
  };
      ...
      <Card>
        ...
        <div style={{ background: "rgba(124,58,237,0.07)", border: "1.5px solid rgba(124,58,237,0.2)", borderRadius: 12, padding: "14px 16px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 28, fontWeight: 900, color: "#7c3aed" }}>{fmt(insurance)}</div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: "#ea580c" }}>{fmt(insurance * 12)}</div>
              ...
              <button key={tier.id} onClick={() => setInsuranceTier(tier.id)} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ textAlign: "right", marginLeft: 10, flexShrink: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 800, color: isSelected ? "#7c3aed" : C.text }}>{fmt(tierCost)}/mo</div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {RECORD_FACTORS.map(r => {
                  ...
                  return (
                    <button key={r.id} onClick={() => setDriverRecord(r.id)} style={{ textAlign: "left", padding: "9px 12px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", background: isSelected ? `${r.badgeColor}14` : "#fafafa", border: `1.5px solid ${isSelected ? r.badgeColor : C.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {DISCOUNTS.map(d => {
                  ...
                  return (
                    <button key={d.id} onClick={() => toggleDiscount(d.id)} style={{ textAlign: "left", padding: "9px 12px", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", background: isOn ? "rgba(15,118,110,0.08)" : "#fafafa", border: `1.5px solid ${isOn ? "#0f766e" : C.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
...
function ProjectSim({ user, onSave }) {
  ...
  const handleSave = () => {
    if (!user) return alert("Create an account to save scenarios!");
    onSave({ id: Date.now().toString(), type: "project", label: `Project — ${fmt(total)} total`, date: new Date().toLocaleDateString(), data: { materials, labor, timeCost, hidden, bufferAmt, total, score } });
  };
...
function RecessionSim({ user, onSave }) {
  ...
  const handleSave = () => {
    if (!user) return alert("Create an account to save scenarios!");
    onSave({ id: Date.now().toString(), type: "recession", label: `Recession Prep — ${runway >= 99 ? "∞" : runway + "mo"} runway`, date: new Date().toLocaleDateString(), data: { income, savings, essentials, drop, reserve, reduced, gap, runway, score } });
  };
...
function AuthModal({ onClose, onAuth }) {
  ...
  const handleSubmit = () => {
    if (!email) return;
    const user = { id: Date.now().toString(), name: name || email.split("@")[0], email, zip, joined: new Date().toLocaleDateString() };
    DB.setUser(user);
    onAuth(user);
    onClose();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.card, borderRadius: "24px 24px 0 0", padding: 28, width: "100%", maxWidth: 480, boxShadow: "0 -20px 60px rgba(2,6,23,0.15)" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          <Btn onClick={() => setMode("signup")} variant={mode === "signup" ? "primary" : "outline"} style={{ flex: 1 }}>Create Account</Btn>
          <Btn onClick={() => setMode("login")} variant={mode === "login" ? "primary" : "outline"} style={{ flex: 1 }}>Log In</Btn>
        </div>
        ...
        {mode === "signup" && <Input label="ZIP Code (for local defaults)" value={zip} onChange={setZip} placeholder="Optional" />}
        <Btn onClick={handleSubmit} style={{ width: "100%", padding: "14px 18px", fontSize: 15 }}>
          {mode === "signup" ? "Create Account & Save Scenarios" : "Log In"}
        </Btn>
        <p style={{ fontSize: 11, color: C.muted, textAlign: "center", marginTop: 12 }}>No password needed for this prototype · Data stays on your device</p>
      </div>
    </div>
  );
}
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
const DB_DASH = {
  get: (k) => { try { return JSON.parse(localStorage.getItem("re_dash_" + k) || "null"); } catch { return null; } },
  set: (k, v) => { try { localStorage.setItem("re_dash_" + k, JSON.stringify(v)); } catch {} },
};
...
function SavingsBucket({ bucket, onUpdate, onDelete }) {
  ...
  return (
    <div style={{ background: "#fafafa", border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
      {editing ? (
        <div>
...
function CDTracker({ cd, onUpdate, onDelete }) {
  ...
  return (
    <div style={{ background: "#fafafa", border: `1.5px solid ${isMatured ? "#0f766e" : C.border}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
      {editing ? (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            <div>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: "#0f766e" }}>{fmt(projectedValue)}</div>
      ...
      <Card style={{ background: "linear-gradient(135deg, #eff6ff, #f0fdf4)", padding: "16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 14 }}>
          ...
          <div style={{ background: "rgba(15,118,110,0.08)", borderRadius: 10, padding: "10px 12px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 900, color: C.green }}>{fmt(totalMonthlySavings)}</div>
          </div>
          <div style={{ background: "rgba(124,58,237,0.08)", borderRadius: 10, padding: "10px 12px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 900, color: "#7c3aed" }}>{fmt(totalCDValue)}</div>
          </div>
        </div>
      </Card>
      ...
      {activeTab === "income" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card>
            ...
            <div style={{ position: "relative", marginBottom: 14 }}>
              ...
              <input type="number" value={monthlyIncome} onChange={e => saveIncome(parseFloat(e.target.value) || 0)}
                style={{ width: "100%", padding: "12px 14px 12px 26px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 15, fontFamily: "inherit", outline: "none", color: C.text, background: "#f8fafc", boxSizing: "border-box", fontWeight: 700 }} />
            </div>
            <div style={{ background: "rgba(37,99,235,0.06)", borderRadius: 10, padding: "10px 14px" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 800, color: C.primary }}>{fmt(monthlyIncome * 12)}/yr</div>
            </div>
          </Card>
          ...
          <Card>
            ...
            {showAddIncome && (
              <div style={{ background: "#f8fafc", borderRadius: 12, padding: 14, marginBottom: 14, border: `1.5px solid ${C.primary}` }}>
            ...
            {incomeStreams.length > 0 && (
              <div style={{ marginTop: 12, background: "rgba(15,118,110,0.07)", borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 900, color: C.green }}>{fmt(totalMonthlyIncome)}</span>
              </div>
            )}
          </Card>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px", textAlign: "center", boxShadow: C.shadow }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 900, color: C.green }}>{fmt(totalSavingsSaved)}</div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px", textAlign: "center", boxShadow: C.shadow }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 900, color: C.primary }}>{fmt(totalMonthlySavings)}</div>
          ...
          {cds.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px", textAlign: "center", boxShadow: C.shadow }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 900, color: C.text }}>{fmt(totalCDPrincipal)}</div>
              </div>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px", textAlign: "center", boxShadow: C.shadow }}>
                <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>Projected Value</div>
                <div style={{ fontSize: 20, fontWeight: 900, color: C.green }}>{fmt(totalCDValue)}</div>
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input value={amount} onChange={e => setAmount(e.target.value)} inputMode="decimal" placeholder="0.00"
                  style={{ width: "100%", padding: "11px 14px 11px 26px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 14, fontFamily: "inherit", outline: "none", color: C.text, background: "#f8fafc", boxSizing: "border-box" }} />
              </div>
            </div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Date</div>
              <input type="date" value={date} onChange={e => setDate(e.target.value)}
                style={{ width: "100%", padding: "11px 12px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 13, fontFamily: "inherit", outline: "none", color: C.text, background: "#f8fafc", boxSizing: "border-box" }} />
            </div>
          </div>
        ...
        {debts.map(d => {
          ...
          return (
            <div key={d.id} style={{ borderRadius: 12, border: `1.5px solid ${C.border}`, marginBottom: 8, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", background: isEditing ? `${col}08` : "#fafafa" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
          {[
            ...
            <button key={s.id} onClick={() => setStrategy(s.id)} style={{
              ...
              background: strategy === s.id ? C.primaryLight : "#fafafa",
...
function NewsPage({ user }) {
  ...
  const [state, setState] = useState(savedPrefs?.state || (user?.zip ? "" : ""));
  ...
  const savePrefs = () => {
    const prefs = { state, topics, lifeStage };
    NewsStore.setPrefs(prefs);
    setShowPrefs(false);
    fetchNews(prefs);
  };

  const fetchNews = async (prefs) => {
    const p = prefs || NewsStore.getPrefs();
    if (!p) return;

    // Check cache first (1 hour)
    const cached = NewsStore.getCache();
    if (cached && cached.prefsKey === JSON.stringify(p)) {
      setArticles(cached.articles || []);
      setSummary(cached.summary || "");
      NewsStore.set(cached.newsContext || "");
    ...
    try {
      const stateText = p.state ? `in ${p.state}` : "across the US";
      const topicLabels = p.topics.map(t => NEWS_TOPICS.find(x => x.id === t)?.label).filter(Boolean).join(", ");
      const stageInfo = LIFE_STAGES.find(s => s.id === p.lifeStage);

      const searchPrompt = `You are a financial news aggregator. The user is a ${stageInfo?.label} (${stageInfo?.desc}) ${stateText}.

Find and summarize the LATEST news (past 2 weeks) on these topics: ${topicLabels}.
${p.state ? `Include specific data for ${p.state} where available (rent trends, local job market, housing prices).` : ""}
  ...
  "articles": [
    {
      ...
      "impact": "how this directly affects someone in their situation",
      "sentiment": "positive|negative|neutral",
      "urgency": "high|medium|low",
      "tag": "short category label"
    }
  ],
  "newsContext": "A 3-4 sentence paragraph summarizing the most important current financial conditions relevant to this person. This will be injected into AI prompts to make financial advice timely.",
  "briefing": "A 2-3 sentence personal financial briefing for this user based on their life stage and location. Conversational tone, like a friend summarizing the news."
}
...
Generate 6-8 articles covering the selected topics. Make them realistic, current, and specific. For ${p.state || "the US"}, include local data where relevant.`;
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "You are a financial news curator. Always return valid JSON only, no markdown fences. Search for real, current news before generating the response.",
      ...
      try {
        ...
        parsed = { articles: [], newsContext: "", briefing: "Unable to load news at this time." };
      }

      const articleList = parsed.articles || [];
      const newsCtx = parsed.newsContext || "";
      const briefingText = parsed.briefing || "";

      setArticles(articleList);
      setSummary(briefingText);
      NewsStore.set(newsCtx);
      setLastFetched(new Date());
      NewsStore.setCache({ articles: articleList, summary: briefingText, newsContext: newsCtx, prefsKey: JSON.stringify(p) });

    } catch (e) {
      setSummary("Unable to load news right now. Your AI insights will still work with built-in knowledge.");
    }
    setLoading(false);
  };

  // Auto-fetch on mount if prefs exist
  useEffect(() => {
    const prefs = NewsStore.getPrefs();
    if (prefs) fetchNews(prefs);
  }, []);

  const sentimentColor = { positive: C.green, negative: "#dc2626", neutral: C.primary };
  const urgencyBg = { high: "rgba(220,38,38,0.07)", medium: "rgba(245,158,11,0.07)", low: "#f8fafc" };
  const urgencyBorder = { high: "rgba(220,38,38,0.2)", medium: "rgba(245,158,11,0.25)", low: C.border };

  const filtered = activeFilter === "all" ? articles : articles.filter(a => a.topic === activeFilter);
  const savedPrefsObj = NewsStore.getPrefs();

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

      {/* Header card */}
      <Card style={{ background: "linear-gradient(135deg, #0f172a, #1e3a5f)", padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, color: "#fff", marginBottom: 4 }}>
              {savedPrefsObj?.state || "United States"}
            </div>
            <div style={{ fontSize: 11, color: "#94a3b8" }}>
              {savedPrefsObj ? LIFE_STAGES.find(s => s.id === savedPrefsObj.lifeStage)?.label + " · " : ""}
              {lastFetched ? `Updated ${lastFetched.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Not yet loaded"}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => fetchNews()} disabled={loading} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", borderRadius: 10, padding: "8px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              {loading ? "⟳" : "↻ Refresh"}
            </button>
            <button onClick={() => setShowPrefs(v => !v)} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", borderRadius: 10, padding: "8px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              ⚙ Prefs
            </button>
          </div>
        </div>
        ...
        <div style={{ background: "rgba(37,99,235,0.25)", borderRadius: 10, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ fontSize: 11, color: "#bfdbfe" }}>
            {NewsStore.get() ? "✦ AI insights are using live news context" : "Load news to enhance AI insights with current data"}
          </div>
        </div>
      </Card>
      ...
      {showPrefs && (
        <Card style={{ border: `1.5px solid ${C.primary}` }}>
          ...
          {/* State picker */}
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: C.muted, marginBottom: 6 }}>Your State (for local data)</div>
            <select value={state} onChange={e => setState(e.target.value)}
              style={{ width: "100%", padding: "11px 14px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 13, fontFamily: "inherit", background: "#fff", color: C.text }}>
              <option value="">🇺🇸 National (no state filter)</option>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {LIFE_STAGES.map(s => (
                <button key={s.id} onClick={() => setLifeStage(s.id)} style={{
                  textAlign: "left", padding: "10px 14px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit",
                  background: lifeStage === s.id ? C.primaryLight : "#fafafa",
                  border: `1.5px solid ${lifeStage === s.id ? C.primary : C.border}`,
                }}>
              ...
              {NEWS_TOPICS.map(t => {
                ...
                return (
                  <button key={t.id} onClick={() => toggleTopic(t.id)} style={{
                    textAlign: "left", padding: "10px 14px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit",
                    background: on ? "rgba(15,118,110,0.07)" : "#fafafa",
          ...
          <Btn onClick={savePrefs} style={{ width: "100%", padding: "13px" }}>
            Save & Load My News Feed
          </Btn>
        </Card>
      )}
      ...
      {loading && (
        <Card style={{ textAlign: "center", padding: 40 }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 8 }}>Searching live news...</div>
          <div style={{ fontSize: 12, color: C.muted }}>Pulling current data for your topics and location</div>
          <div style={{ marginTop: 16, display: "flex", justifyContent: "center", gap: 6 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ width: 8, height: 8, borderRadius: 999, background: C.primary, opacity: 0.4,
      ...
      {!loading && summary && (
        <div style={{ background: "linear-gradient(135deg, #eff6ff, #f0fdf4)", border: "1px solid rgba(37,99,235,0.15)", borderRadius: 16, padding: 16 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, textTransform: "uppercase", marginBottom: 4 }}>Your Personal Briefing</div>
              <div style={{ fontSize: 13, color: "#1e3a5f", lineHeight: 1.7 }}>{summary}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {!loading && articles.length > 0 && (
        <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2 }}>
          <button onClick={() => setActiveFilter("all")} style={{ padding: "6px 14px", borderRadius: 999, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap",
            background: activeFilter === "all" ? C.primary : C.card, color: activeFilter === "all" ? "#fff" : C.muted, border: `1.5px solid ${activeFilter === "all" ? C.primary : C.border}` }}>
            All
          </button>
          {NEWS_TOPICS.filter(t => articles.some(a => a.topic === t.id)).map(t => (
            <button key={t.id} onClick={() => setActiveFilter(t.id)} style={{ padding: "6px 14px", borderRadius: 999, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap",
              background: activeFilter === t.id ? C.primary : C.card, color: activeFilter === t.id ? "#fff" : C.muted, border: `1.5px solid ${activeFilter === t.id ? C.primary : C.border}` }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
      ...
      {!loading && filtered.map((article, i) => {
        const topicInfo = NEWS_TOPICS.find(t => t.id === article.topic);
        const sColor = sentimentColor[article.sentiment] || C.muted;
        const uBg = urgencyBg[article.urgency] || "#f8fafc";
        const uBorder = urgencyBorder[article.urgency] || C.border;
        return (
          <div key={article.id || i} style={{ background: uBg, border: `1.5px solid ${uBorder}`, borderRadius: 16, padding: "16px", position: "relative", overflow: "hidden" }}>
            {/* Urgency accent bar */}
            {article.urgency === "high" && <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 4, background: "#dc2626", borderRadius: "16px 0 0 16px" }} />}

            <div style={{ paddingLeft: article.urgency === "high" ? 8 : 0 }}>
              {/* Header row */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, gap: 8 }}>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: topicInfo?.icon ? C.muted : C.muted, background: C.card, border: `1px solid ${C.border}`, padding: "2px 8px", borderRadius: 999 }}>
                    {topicInfo?.icon} {article.tag || topicInfo?.label}
                  </span>
                  {article.urgency === "high" && (
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#dc2626", background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.2)", padding: "2px 8px", borderRadius: 999 }}>⚡ High Impact</span>
                  )}
                </div>
                <div style={{ width: 8, height: 8, borderRadius: 999, background: sColor, flexShrink: 0, marginTop: 3 }} title={article.sentiment} />
              </div>
              ...
              {/* Summary */}
              <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.6, marginBottom: 8 }}>{article.summary}</div>

              {/* Impact callout */}
              {article.impact && (
                <div style={{ background: "rgba(37,99,235,0.06)", border: "1px solid rgba(37,99,235,0.12)", borderRadius: 9, padding: "8px 12px", display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 13, flexShrink: 0 }}>💡</span>
                  <div style={{ fontSize: 11, color: C.primary, lineHeight: 1.5, fontWeight: 500 }}>{article.impact}</div>
                </div>
              )}
            </div>
          </div>
      ...
      {!loading && !articles.length && !showPrefs && (
        <Card style={{ textAlign: "center", padding: 40 }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📰</div>
          <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 8 }}>No news loaded yet</div>
          <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>Set your preferences to get a personalized feed that also powers your AI insights.</div>
          <Btn onClick={() => setShowPrefs(true)} style={{ width: "100%" }}>Set Up My News Feed</Btn>
        </Card>
      )}
...
export default function RealityEstimator() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "ui-sans-serif, system-ui, -apple-system, 'Segoe UI', sans-serif", color: C.text, paddingBottom: 80 }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(246,248,251,0.88)", borderBottom: `1px solid ${C.border}`, backdropFilter: "saturate(140%) blur(10px)" }}>
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "12px 16px", display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontWeight: 900, fontSize: 14, lineHeight: 1.2 }}>Reality Estimator</div>
      ...
      {/* Bottom Nav */}
      <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, background: "rgba(255,255,255,0.94)", borderTop: `1px solid ${C.border}`, backdropFilter: "blur(10px)", display: "flex", justifyContent: "space-around", padding: "10px 0 14px", zIndex: 50 }}>
        {navItems.map(n => (
