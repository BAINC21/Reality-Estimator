...
const STRIPE_PRICE_ID = "price_1TB2NkF3rV02A1d4VjnjwcfC";
...
const startCheckout = async (user) => {
  try {
    ...
    if (data.url) {
      ...
      alert("Checkout error: " + (data.error || "Unknown error. Check Vercel logs."));
    }
  } catch (err) {
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        ...
        <input
          ...
          onBlur={e => { handleBlur(); e.target.style.borderColor = C.border; }}
          inputMode="decimal"
          style={{
...
function ProModal({ onClose, onUpgrade }) {
  const features = [
    { icon: "📂", title: "Unlimited Scenarios", desc: "Save as many simulations as you need" },
    { icon: "✦", title: "AI Insights on Every Sim", desc: "Get honest AI analysis on all simulators" },
    { icon: "📱", title: "Cross-Device Sync", desc: "Access your data from any device" },
    { icon: "🧾", title: "Full Spending History", desc: "Track up to 100 transactions over 3 months" },
    { icon: "🚀", title: "Early Access", desc: "First to get new tools and simulators" },
  ];
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.card, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 480, boxShadow: "0 -20px 60px rgba(0,0,0,0.4)", overflow: "hidden" }}>
        {/* Header */}
        <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a5f)", padding: "28px 24px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: "#93c5fd", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>⚡ Upgrade</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#fff" }}>Reality Estimator Pro</div>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 16 }}>
            <span style={{ fontSize: 36, fontWeight: 900, color: "#f59e0b" }}>$4.99</span>
            <span style={{ fontSize: 14, color: "#94a3b8" }}>/month</span>
          </div>
          <button onClick={onUpgrade}
            style={{ width: "100%", padding: "14px", borderRadius: 12, background: "#f59e0b", border: "none", color: "#0f172a", fontWeight: 900, fontSize: 16, cursor: "pointer", fontFamily: "inherit" }}>
            Upgrade to Pro
          </button>
          <div style={{ fontSize: 11, color: "#64748b", textAlign: "center", marginTop: 8 }}>Cancel anytime · No contracts</div>
        </div>
        {/* Features */}
        <div style={{ padding: "20px 24px 32px" }}>
          <div style={{ fontSize: 12, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Everything in Pro</div>
          {features.map(f => (
            <div key={f.title} style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 16 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #f59e0b22, #f59e0b11)", border: "1px solid #f59e0b33", display: "grid", placeItems: "center", fontSize: 17, flexShrink: 0 }}>{f.icon}</div>
              <div>
...
// ─── LOCKED AI INSIGHT ────────────────────────────────────────────────────────
const LockedAIInsight = ({ onUpgrade }) => (
  <div style={{ background: C.bg === "#0a0f1e" ? `linear-gradient(135deg, ${C.card}, #1e3a5f)` : "linear-gradient(135deg, #eff6ff, #f0fdf4)", border: `1px solid ${C.primary}26`, borderRadius: 14, padding: 14, marginTop: 14 }}>
    <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
      <div style={{ width: 26, height: 26, borderRadius: 8, background: "linear-gradient(135deg, #f59e0b, #d97706)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0, color: "#fff", fontWeight: 700 }}>⚡</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5, marginBottom: 8, filter: "blur(3px)", userSelect: "none" }}>Upgrade to Pro to unlock AI insights that analyze your numbers and give you honest, personalized feedback on your financial decisions...</div>
        <button onClick={onUpgrade} style={{ fontSize: 12, fontWeight: 700, color: "#f59e0b", background: "#f59e0b18", border: "1px solid #f59e0b44", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontFamily: "inherit" }}>
...
// ─── SIMULATORS ───────────────────────────────────────────────────────────────
function MovingOutSim({ user, onSave, onShowPro }) {
  ...
  const handleSave = () => {
    ...
    const label = scenarioName.trim() ? scenarioName.trim() : `Moving Out — ${fmt(total)}/mo`;
    onSave({ id: Date.now().toString(), type: "moving", label, date: new Date().toLocaleDateString(), data: { income, rent, utilities, groceries, insurance, subscriptions, furniture, parking, savings, total, ratio, surplus, score } });
    setScenarioName("");
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card>
        ...
        {user?.is_pro !== false ? <AIInsight text={aiText} loading={aiLoading} /> : <LockedAIInsight onUpgrade={onShowPro} />}
        <div style={{ marginTop: 10 }}>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Scenario Name (optional)</div>
          <input value={scenarioName} onChange={e => setScenarioName(e.target.value)} placeholder="e.g. My First Apartment, NYC Move"
...
function CarOwnershipSim({ user, onSave, onShowPro }) {
  const [scenarioName, setScenarioName] = useState("");
  const [income, setIncome] = useState(4000);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card>
        ...
        {showHidden && <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
          {[[`Registration / Taxes (monthly)`, registration, setRegistration], [`Tolls`, tolls, setTolls], [`Parking`, parking2, setParking2]].map(([label, val, setter]) => (
            <div key={label} style={{ display: "flex", alignItems: "flex-end", gap: 10, marginBottom: 4 }}>
              <div style={{ flex: 1 }}>
                <NumInput label={label} value={val} onChange={setter} accentColor="#7c3aed" />
              </div>
              <div style={{ textAlign: "right", paddingBottom: 12, minWidth: 80 }}>
                <div style={{ fontSize: 10, color: C.muted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>Annual</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#7c3aed" }}>{fmt(val * 12)}</div>
              </div>
            </div>
          ))}
        </div>}
...
function ProjectSim({ user, onSave, onShowPro }) {
  const [scenarioName, setScenarioName] = useState("");
...
function RecessionSim({ user, onSave, onShowPro }) {
  const [scenarioName, setScenarioName] = useState("");
  const [income, setIncome] = useState(4000);
    ...
    try {
      if (mode === "signup") {
        try {
          ...
          const user = { id: sbUser.id, name: name || email.split("@")[0], email: sbUser.email, zip, is_admin: false, is_pro: false, joined: new Date().toLocaleDateString() };
          DB.setUser(user);
          onAuth(user);
          onClose();
        } catch (e) {
        ...
        const user = { id: sbUser.id, name: profile?.name || sbUser.email.split("@")[0], email: sbUser.email, zip: profile?.zip || "", is_admin: profile?.is_admin || false, is_pro: profile?.is_pro || false, joined: new Date(sbUser.created_at).toLocaleDateString() };
        DB.setUser(user);
        onAuth(user);
        onClose();
      }
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              style={{ width: "100%", padding: "11px 44px 11px 14px", border: `1.5px solid ${C.border}`, borderRadius: 12, fontSize: 14, fontFamily: "inherit", outline: "none", color: C.text, background: C.cardAlt, boxSizing: "border-box" }}
            />
            <button
...
function SavingsBucket({ bucket, onUpdate, onDelete }) {
  ...
  const [adjustMode, setAdjustMode] = useState("add"); // "add" or "withdraw"
  ...
  const monthsLeft = bucket.monthly > 0 && remaining > 0 ? Math.ceil(remaining / bucket.monthly) : remaining === 0 ? 0 : null;
  ...
  const handleAdjust = () => {
    ...
    if (!amt || amt <= 0) return;
    let newSaved = adjustMode === "add" ? bucket.saved + amt : Math.max(0, bucket.saved - amt);
    onUpdate({ ...bucket, saved: Math.round(newSaved * 100) / 100 });
    setAdjustAmt("");
    setAdjustNote("");
    setShowAdjust(false);
  };
    ...
    <div style={{ background: C.cardAlt, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
      {editing ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            ...
            <div>
              ...
              <select value={bucket.type} onChange={e => onUpdate({ ...bucket, type: e.target.value })}
                style={{ width: "100%", padding: "8px 10px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontFamily: "inherit", background: C.cardAlt, color: C.text }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                ...
                {monthsLeft === 0
                  ? <span style={{ color: C.green }}> · 🎉 Goal reached!</span>
                  : monthsLeft
                    ? <span> · ~{monthsLeft} mo to go</span>
                    : null}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ fontSize: 17, fontWeight: 600, color }}>{pct}%</div>
              {/* + button */}
              <button onClick={() => { setShowAdjust(v => !v); setAdjustAmt(""); setAdjustMode("add"); }}
                style={{ width: 28, height: 28, borderRadius: 8, background: showAdjust ? color : `${color}22`, border: `1.5px solid ${color}`, color: showAdjust ? "#fff" : color, fontSize: 18, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>
                {showAdjust ? "×" : "+"}
              </button>
              <button onClick={() => setEditing(true)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 15, padding: 4 }}>✏️</button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.muted, marginBottom: showAdjust ? 12 : 0 }}>
          ...
          {/* Adjust popup */}
          {showAdjust && (
            <div style={{ background: C.bg, border: `1.5px solid ${color}44`, borderRadius: 12, padding: 14, marginTop: 4 }}>
              {/* Add / Withdraw toggle */}
              <div style={{ display: "flex", gap: 6, marginBottom: 12, background: C.cardAlt, borderRadius: 8, padding: 3 }}>
                {[["add", "➕ Deposit"], ["withdraw", "➖ Withdraw"]].map(([mode, label]) => (
                  <button key={mode} onClick={() => setAdjustMode(mode)}
                    style={{ flex: 1, padding: "7px 0", borderRadius: 6, border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 12,
                      background: adjustMode === mode ? (mode === "add" ? C.green : C.red) : "transparent",
                      color: adjustMode === mode ? "#fff" : C.muted, transition: "all 0.15s" }}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input type="number" value={adjustAmt} onChange={e => setAdjustAmt(e.target.value)}
                    ...
                    style={{ width: "100%", padding: "10px 12px 10px 24px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                </div>
              </div>
              ...
              {adjustAmt && parseFloat(adjustAmt) > 0 && (() => {
                const amt = parseFloat(adjustAmt);
                const newSaved = adjustMode === "add" ? bucket.saved + amt : Math.max(0, bucket.saved - amt);
                const newRemaining = Math.max(0, bucket.target - newSaved);
                const newMonths = bucket.monthly > 0 && newRemaining > 0 ? Math.ceil(newRemaining / bucket.monthly) : 0;
                const newPct = Math.min(100, bucket.target > 0 ? Math.round((newSaved / bucket.target) * 100) : 0);
                return (
                  <div style={{ background: `${color}11`, border: `1px solid ${color}33`, borderRadius: 8, padding: "8px 12px", marginBottom: 10, fontSize: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: C.muted }}>New balance</span>
                      <span style={{ fontWeight: 700, color }}>{fmt(newSaved)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                      <span style={{ color: C.muted }}>Progress</span>
                      <span style={{ fontWeight: 700, color }}>{newPct}%</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: C.muted }}>New timeline</span>
                      <span style={{ fontWeight: 700, color: newMonths === 0 ? C.green : color }}>
                        {newMonths === 0 ? "🎉 Goal reached!" : `~${newMonths} months`}
                      </span>
                    </div>
                  </div>
                );
              ...
              {/* Confirm button */}
              <Btn onClick={handleAdjust}
                style={{ width: "100%", padding: "11px", background: adjustMode === "add" ? C.green : C.red, fontSize: 13 }}>
                {adjustMode === "add" ? "✓ Confirm Deposit" : "✓ Confirm Withdrawal"}
              </Btn>
            </div>
          )}
        </div>
...
function Dashboard({ user, onLogout, onShowAuth, showEmail, onToggleEmail, onShowPro }) {
  ...
  // Persist helpers
  const syncDashboard = (patch) => {
    if (user?.id) SB.saveDashboard(user.id, patch).catch(() => {});
  };

  const saveIncome = (v) => { setMonthlyIncome(v); DB_DASH.set("income", v); syncDashboard({ income: v }); };
  const saveStreams = (v) => { setIncomeStreams(v); DB_DASH.set("incomeStreams", v); syncDashboard({ income_streams: v }); };
  const saveBuckets = (v) => { setBuckets(v); DB_DASH.set("buckets", v); syncDashboard({ buckets: v }); };
  const saveCDs = (v) => { setCDs(v); DB_DASH.set("cds", v); syncDashboard({ cds: v }); };
  const saveBills = (v) => { setBills(v); DB_DASH.set("bills", v); syncDashboard({ bills: v }); };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card style={{ background: C.bg === "#0a0f1e" ? `linear-gradient(135deg, ${C.card}, #1e3a5f)` : "linear-gradient(135deg, #eff6ff, #f0fdf4)", padding: "20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {user.is_pro && <span style={{ fontSize: 10, fontWeight: 900, color: "#0f172a", background: "linear-gradient(135deg, #f59e0b, #d97706)", borderRadius: 5, padding: "2px 7px", letterSpacing: "0.04em", flexShrink: 0 }}>PRO</span>}
            </div>
        ...
        {!user.is_pro && (
          <a href="#" onClick={e => { e.preventDefault(); onShowPro && onShowPro(); }}
            style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, padding: "10px 14px", borderRadius: 10, background: "linear-gradient(135deg, #f59e0b22, #f59e0b11)", border: "1px solid #f59e0b44", textDecoration: "none" }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 800, color: "#f59e0b" }}>⚡ Upgrade to Pro</div>
              <div style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>Unlimited saves · AI on every sim · $4.99/mo</div>
            </div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#f59e0b" }}>→</div>
          </a>
        )}
      </Card>
      ...
      {activeTab === "overview" && (() => {
        ...
        const totalBills = n(bills.rent) + n(bills.carPayment)
          + bills.insurances.reduce((s, i) => s + n(i.amount), 0)
          + bills.others.reduce((s, o) => s + n(o.amount), 0);
        const freeCash = Math.max(0, totalMonthlyIncome - totalBills - totalMonthlySavings);
        const committedRatio = totalMonthlyIncome > 0 ? Math.round(((totalBills + totalMonthlySavings) / totalMonthlyIncome) * 100) : 0;
        const savingsRatio = totalMonthlyIncome > 0 ? Math.round((totalMonthlySavings / totalMonthlyIncome) * 100) : 0;
        const recSim = [...scenarios].reverse().find(s => s.type === "recession");
        const runway = recSim?.data?.runway;
        const avgScore = scenarios.length > 0 ? Math.round(scenarios.reduce((s, sc) => s + (sc.data?.score || 0), 0) / scenarios.length) : null;
        const portfolioTier = !avgScore ? { label: "No Data", color: C.muted }
          : avgScore >= 75 ? { label: "Strong", color: C.green }
          : avgScore >= 55 ? { label: "Stable", color: "#0284c7" }
          : avgScore >= 35 ? { label: "Building", color: "#d97706" }
          : { label: "Needs Attention", color: "#e07b3a" };
        const score = avgScore || 0;
        const R = 38, circ = 2 * Math.PI * R;

        const labelRows = [
          { icon: "💰", label: "Monthly Income", value: fmt(totalMonthlyIncome), color: C.primary },
          { icon: "🧾", label: "Total Bills", value: totalBills > 0 ? fmt(totalBills) + "/mo" : "—", color: "#e07b3a" },
          { icon: "🏠", label: "Rent / Mortgage", value: n(bills.rent) > 0 ? fmt(n(bills.rent)) + "/mo" : "—", color: "#0284c7" },
          { icon: "🚗", label: "Car Payment", value: n(bills.carPayment) > 0 ? fmt(n(bills.carPayment)) + "/mo" : "—", color: "#d97706" },
          { icon: "🛡️", label: "Insurance", value: bills.insurances.reduce((s, i) => s + n(i.amount), 0) > 0 ? fmt(bills.insurances.reduce((s, i) => s + n(i.amount), 0)) + "/mo" : "—", color: "#7c3aed" },
          { icon: "💚", label: "Saving/mo", value: fmt(totalMonthlySavings), color: C.green },
          { icon: "🆓", label: "Free Cash", value: fmt(freeCash) + "/mo", color: freeCash > 500 ? C.green : freeCash > 0 ? "#d97706" : C.red },
          { icon: "🏦", label: "Crisis Runway", value: runway >= 99 ? "Indefinite" : runway > 0 ? `${runway} months` : "—", color: runway >= 6 ? C.green : runway >= 3 ? "#d97706" : C.muted },
          ...scenarios.map(s => ({
            icon: icons[s.type] || "📋",
            label: s.label,
            value: `Score ${s.data?.score || 0}`,
            color: (s.data?.score || 0) >= 75 ? C.green : (s.data?.score || 0) >= 50 ? "#d97706" : "#e07b3a",
          })),
        ];
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* PORTFOLIO CARD — ring left, labels right */}
            <Card>
              <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Portfolio Overview</div>
                ...
                <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                  <div style={{ position: "relative", width: 92, height: 92 }}>
                    <svg width="92" height="92" viewBox="0 0 92 92">
                      <circle cx="46" cy="46" r={R} fill="none" stroke={C.border} strokeWidth="8" />
                      <circle cx="46" cy="46" r={R} fill="none" stroke={portfolioTier.color} strokeWidth="8"
                    ...
                    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ fontSize: avgScore ? 22 : 14, fontWeight: 700, color: portfolioTier.color, lineHeight: 1 }}>{avgScore || "—"}</div>
                      <div style={{ fontSize: 8, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 2 }}>avg</div>
                    </div>
                  </div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: portfolioTier.color, textAlign: "center" }}>{portfolioTier.label}</div>
                  <div style={{ fontSize: 10, color: C.muted, textAlign: "center" }}>{committedRatio}% committed<br />{savingsRatio}% saving</div>
                </div>

                {/* Right — label rows */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  {labelRows.map((row, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: i < labelRows.length - 1 ? `1px solid ${C.border}` : "none" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 12 }}>{row.icon}</span>
                        <span style={{ fontSize: 12, color: C.muted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 130 }}>{row.label}</span>
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 600, color: row.color, flexShrink: 0 }}>{row.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {buckets.length > 0 && (
              <Card>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Savings Goals</div>
                {buckets.map(b => {
                  const pct = Math.min(100, b.target > 0 ? Math.round((b.saved / b.target) * 100) : 0);
                  const colorMap = { emergency: "#0f766e", vacation: "#0284c7", car: "#d97706", home: "#7c3aed", other: "#6b7280" };
                  const col = colorMap[b.type] || "#6b7280";
                  const moLeft = b.monthly > 0 && b.target > b.saved ? Math.ceil((b.target - b.saved) / b.monthly) : null;
                  return (
                    <div key={b.id} style={{ marginBottom: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{b.label}</span>
                        <span style={{ fontSize: 12, fontWeight: 600, color: col }}>{pct}% · {fmt(b.saved)}{moLeft ? ` · ~${moLeft}mo` : ""}</span>
                      </div>
      ...
      {activeTab === "bills" && (() => {
        const n = (v) => parseFloat(v) || 0;
        const totalInsurance = bills.insurances.reduce((s, i) => s + n(i.amount), 0);
        const totalOther = bills.others.reduce((s, o) => s + n(o.amount), 0);
        const totalBills = n(bills.rent) + n(bills.carPayment) + totalInsurance + totalOther;
        const income = monthlyIncome + incomeStreams.reduce((s, x) => s + (x.type === "annual" ? x.amount / 12 : x.amount), 0);
        const ratio = income > 0 ? Math.round((totalBills / income) * 100) : 0;
        // Gentle score: 0-100, comfortable vs tight only
        const score = Math.max(0, Math.min(100,
          ratio <= 40 ? 100 - ratio :
          ratio <= 60 ? 60 - (ratio - 40) :
          Math.max(0, 40 - (ratio - 60) * 2)
        ));
        const tier = score >= 60
          ? { label: "Comfortable", color: C.green, bg: "rgba(15,118,110,0.08)", border: "rgba(15,118,110,0.25)" }
          : score >= 30
            ? { label: "Manageable", color: "#d97706", bg: "rgba(217,119,6,0.08)", border: "rgba(217,119,6,0.25)" }
            : { label: "Tight", color: "#e07b3a", bg: "rgba(224,123,58,0.08)", border: "rgba(224,123,58,0.25)" };

        const BillInput = ({ label, value, onChange, placeholder = "0" }) => (
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>{label}</div>
            <div style={{ position: "relative" }}>
              <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 14 }}>$</span>
              <input type="number" defaultValue={value} onBlur={e => onChange(e.target.value)} placeholder={placeholder}
                style={{ width: "100%", padding: "13px 12px 13px 28px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 16, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
            </div>
          </div>
        );
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* Score + summary */}
            <Card style={{ background: tier.bg, border: `1.5px solid ${tier.border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                {/* Custom score ring — gentle labels only */}
                <div style={{ position: "relative", width: 90, height: 90, flexShrink: 0 }}>
                  <svg width="90" height="90" viewBox="0 0 90 90">
                    <circle cx="45" cy="45" r="35" fill="none" stroke={C.border} strokeWidth="8" />
                    <circle cx="45" cy="45" r="35" fill="none" stroke={tier.color} strokeWidth="8"
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ fontSize: 20, fontWeight: 600, color: tier.color, lineHeight: 1 }}>{score}</div>
                    <div style={{ fontSize: 9, color: C.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 2 }}>{tier.label}</div>
                  </div>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, color: C.text, marginBottom: 4 }}>Monthly Bills</div>
                  <div style={{ fontSize: 22, fontWeight: 600, color: tier.color, marginBottom: 4 }}>{fmt(totalBills)}<span style={{ fontSize: 12, color: C.muted, fontWeight: 400 }}>/mo</span></div>
                  <div style={{ fontSize: 12, color: C.muted }}>{ratio}% of your monthly income</div>
                </div>
              </div>
            </Card>
            ...
            {/* Housing */}
            <Card>
              <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>🏠 Housing</div>
              <BillInput label="Monthly Rent / Mortgage" value={bills.rent} onChange={v => saveBills({ ...bills, rent: v })} />
            </Card>

            {/* Vehicle */}
            <Card>
              <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>🚗 Vehicle</div>
              <BillInput label="Car Payment" value={bills.carPayment} onChange={v => saveBills({ ...bills, carPayment: v })} />
            </Card>

            {/* Insurance */}
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>🛡️ Insurance</div>
                <button onClick={() => saveBills({ ...bills, insurances: [...bills.insurances, { id: Date.now().toString(), label: "", amount: "" }] })}
                  style={{ width: 26, height: 26, borderRadius: 7, background: `${C.primary}22`, border: `1.5px solid ${C.primary}`, color: C.primary, fontSize: 16, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              </div>
              {bills.insurances.map((ins) => (
                <div key={ins.id} style={{ marginBottom: 12, background: C.bg, borderRadius: 10, padding: "10px 12px", border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>Insurance Type</div>
                    {bills.insurances.length > 1 && (
                      <button onClick={() => saveBills({ ...bills, insurances: bills.insurances.filter(x => x.id !== ins.id) })}
                        style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 16, padding: 2 }}>✕</button>
                    )}
                  </div>
                  <input
                    defaultValue={ins.label}
                    onBlur={e => { const updated = bills.insurances.map(x => x.id === ins.id ? { ...x, label: e.target.value } : x); saveBills({ ...bills, insurances: updated }); }}
                    placeholder="e.g. Car Insurance, Health, Dental"
                    style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none", marginBottom: 8 }} />
                  <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Monthly Amount</div>
                  <div style={{ position: "relative" }}>
                    <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 14 }}>$</span>
                    <input
                      type="number"
                      defaultValue={ins.amount}
                      onBlur={e => { const updated = bills.insurances.map(x => x.id === ins.id ? { ...x, amount: e.target.value } : x); saveBills({ ...bills, insurances: updated }); }}
                      placeholder="0"
                      style={{ width: "100%", padding: "11px 12px 11px 28px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                  </div>
                </div>
              ))}
              {totalInsurance > 0 && (
                <div style={{ fontSize: 12, color: C.muted, borderTop: `1px solid ${C.border}`, paddingTop: 8, marginTop: 4, display: "flex", justifyContent: "space-between" }}>
                  <span>Total Insurance</span>
                  <span style={{ fontWeight: 600, color: C.text }}>{fmt(totalInsurance)}/mo · {fmt(totalInsurance * 12)}/yr</span>
                </div>
              )}
            </Card>
            ...
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>📦 Other Bills</div>
                <button onClick={() => saveBills({ ...bills, others: [...bills.others, { id: Date.now().toString(), label: "", amount: "" }] })}
                  style={{ width: 26, height: 26, borderRadius: 7, background: `${C.primary}22`, border: `1.5px solid ${C.primary}`, color: C.primary, fontSize: 16, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              </div>
              {bills.others.length === 0 && (
                <div style={{ fontSize: 13, color: C.muted, textAlign: "center", padding: "12px 0" }}>Tap + to add subscriptions, utilities, etc.</div>
              )}
              {bills.others.map((other) => (
                <div key={other.id} style={{ marginBottom: 12, background: C.bg, borderRadius: 10, padding: "10px 12px", border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>Bill Name</div>
                    <button onClick={() => saveBills({ ...bills, others: bills.others.filter(x => x.id !== other.id) })}
                      style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 16, padding: 2 }}>✕</button>
                  </div>
                  <input
                    defaultValue={other.label}
                    onBlur={e => { const updated = bills.others.map(x => x.id === other.id ? { ...x, label: e.target.value } : x); saveBills({ ...bills, others: updated }); }}
                    placeholder="e.g. Netflix, Electric, Gym, Internet"
                    style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none", marginBottom: 8 }} />
                  <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Monthly Amount</div>
                  <div style={{ position: "relative" }}>
                    <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.muted, fontSize: 14 }}>$</span>
                    <input
                      ...
                      onBlur={e => { const updated = bills.others.map(x => x.id === other.id ? { ...x, amount: e.target.value } : x); saveBills({ ...bills, others: updated }); }}
                      placeholder="0"
                      style={{ width: "100%", padding: "11px 12px 11px 28px", border: `1.5px solid ${C.border}`, borderRadius: 10, fontSize: 14, fontFamily: "inherit", color: C.text, background: C.cardAlt, boxSizing: "border-box", outline: "none" }} />
                  </div>
                </div>
              ...
              {totalOther > 0 && (
                <div style={{ fontSize: 12, color: C.muted, borderTop: `1px solid ${C.border}`, paddingTop: 8, marginTop: 4, display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontWeight: 600, color: C.text }}>{fmt(totalOther)}/mo · {fmt(totalOther * 12)}/yr</span>
                </div>
              )}
            </Card>
            ...
            {totalBills > 0 && (
              <Card>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Breakdown</div>
                {[["🏠 Rent / Mortgage", n(bills.rent)], ["🚗 Car Payment", n(bills.carPayment)], ["🛡️ Insurance", totalInsurance], ["📦 Other", totalOther]].filter(([, v]) => v > 0).map(([label, val]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{fmt(val)}/mo</span>
                      <span style={{ fontSize: 11, color: C.muted, marginLeft: 8 }}>{fmt(val * 12)}/yr</span>
                    </div>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 10, marginTop: 4 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: tier.color }}>{fmt(totalBills)}/mo</span>
                    <span style={{ fontSize: 12, color: C.muted, marginLeft: 8 }}>{fmt(totalBills * 12)}/yr</span>
                  </div>
                </div>
              </Card>
            )}
        ...
        const ScenarioDetail = ({ s }) => {
          ...
          if (s.type === "moving") {
            rows.push(
              ...
              ["Groceries", fmt(d.groceries)],
              ["Insurance", fmt(d.insurance)],
              ["Subscriptions", fmt(d.subscriptions)],
              ["Furniture / Setup", fmt(d.furniture)],
              ["Parking", fmt(d.parking)],
              ["Savings Goal", fmt(d.savings)],
              ["Total Monthly", fmt(d.total), true],
              ["% of Income", `${d.ratio}%`, true],
              ["Monthly Surplus", fmt(d.surplus), true],
            );
          } else if (s.type === "car") {
            rows.push(
              ["Monthly Income", fmt(d.income)],
              ["Car Payment", fmt(d.payment)],
              ["Insurance", fmt(d.insurance)],
              ["Gas", fmt(d.gas)],
              ["Maintenance", fmt(d.maintenance)],
              ["Monthly Total", fmt(d.total), true],
              ["% of Income", `${d.ratio}%`, true],
              ["Annual Cost", fmt(d.annualCost), true],
            );
          } else if (s.type === "project") {
            rows.push(
              ["Materials", fmt(d.materials)],
              ["Labor", fmt(d.labor)],
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {scenarios.length === 0 ? (
              ...
              const isOpen = expandedScenarioId === s.id;
              return (
                <Card key={s.id} style={{ borderLeft: `4px solid ${colors[s.type] || C.primary}`, cursor: "pointer" }}
                  onClick={() => setExpandedScenarioId(isOpen ? null : s.id)}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: C.muted }}>Saved {s.date} · {isOpen ? "tap to close" : "tap to view"}</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <ScoreRing score={s.data?.score || 0} size={52} />
                      <div style={{ fontSize: 16, color: C.muted, transition: "transform 0.2s", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)" }}>▾</div>
                      <Btn onClick={e => { e.stopPropagation(); handleDeleteScenario(s.id); }} variant="danger" style={{ padding: "6px 10px", fontSize: 12 }}>✕</Btn>
                    </div>
                  </div>
                  {isOpen && <ScenarioDetail s={s} />}
                </Card>
              );
            })}
          </div>
      ...
      {activeTab === "settings" && (
        ...
        }} onLogout={onLogout} showEmail={showEmail} onToggleEmail={onToggleEmail} />
      )}
      ...
      {user.is_pro ? (
        ...
        <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a5f)", borderRadius: 16, padding: 20, marginBottom: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: "#93c5fd", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>⚡ Upgrade</div>
              <div style={{ fontSize: 18, fontWeight: 900, color: "#fff", marginBottom: 4 }}>Reality Estimator Pro</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#f59e0b" }}>$4.99<span style={{ fontSize: 13, fontWeight: 500, color: "#94a3b8" }}>/mo</span></div>
            </div>
            <div style={{ fontSize: 28 }}>🚀</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
            {["Unlimited saved scenarios", "AI insights on every simulator", "Extended spending history", "Cross-device sync", "Early access to new tools"].map(f => (
          ...
          <a href="#" onClick={e => { e.preventDefault(); startCheckout(user); }}
            style={{ display: "block", width: "100%", padding: "13px", borderRadius: 10, background: "#f59e0b", color: "#0f172a", fontWeight: 900, fontSize: 15, textAlign: "center", textDecoration: "none", boxSizing: "border-box" }}>
            Upgrade to Pro — $4.99/mo
          </a>
          <div style={{ fontSize: 10, color: "#64748b", textAlign: "center", marginTop: 8 }}>Cancel anytime · No contracts</div>
        </div>
      )}
 ...
 const tabs = [
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {active === "moving" && <MovingOutSim user={user} onSave={onSave} onShowPro={onShowPro} />}
      {active === "car" && <CarOwnershipSim user={user} onSave={onSave} onShowPro={onShowPro} />}
...
function SpendingTracker({ user, onShowPro }) {
  const FREE_MAX_TX = 15;
  ...
  const isPro = user?.is_pro !== false && user != null ? true : user?.is_pro || false;

  const addTransaction = () => {
    if (!desc.trim() || !amount || isNaN(parseFloat(amount))) return;
    if (!isPro && transactions.length >= FREE_MAX_TX) {
      onShowPro && onShowPro();
      return;
    }
  ...
  // Free tier: only show last 2 weeks of transactions, max 15
  const cutoff = new Date(Date.now() - FREE_MAX_DAYS * 86400000).toISOString().split("T")[0];
  const visibleTransactions = isPro
    ? transactions
    : transactions.filter(t => t.date >= cutoff).slice(0, FREE_MAX_TX);

  const deleteTransaction = (id) => { persist(transactions.filter(t => t.id !== id)); setAnalyzed(false); };

  const total = visibleTransactions.reduce((s, t) => s + t.amount, 0);
  const byNecessity = NECESSITY.map(n => ({
    ...n,
    amount: visibleTransactions.filter(t => t.necessity === n.id).reduce((s, t) => s + t.amount, 0),
    count: visibleTransactions.filter(t => t.necessity === n.id).length,
  }));
  const byCategory = CATEGORIES.map(c => ({
    ...c,
    amount: visibleTransactions.filter(t => t.category === c.id).reduce((s, t) => s + t.amount, 0),
  })).filter(c => c.amount > 0).sort((a, b) => b.amount - a.amount);

  const filtered = filter === "all" ? visibleTransactions : visibleTransactions.filter(t => t.necessity === filter);

  const runAI = async () => {
    if (visibleTransactions.length === 0) return;
    setAiLoading(true); setAiAnalysis("");
    const summary = visibleTransactions.slice(0, 30).map(t => `${t.date} | ${t.desc} | $${t.amount} | ${t.category} | ${t.necessity}`).join("\n");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {(aiAnalysis || aiLoading) && (
        <div style={{ background: C.bg === "#0a0f1e" ? `linear-gradient(135deg, ${C.card}, #1e3a5f)` : "linear-gradient(135deg, #eff6ff, #f0fdf4)", border: `1px solid ${C.primary}26`, borderRadius: 14, padding: 16 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div style={{ fontSize: 13, color: C.bg === "#0a0f1e" ? "#c8d8f0" : "#1e3a5f", lineHeight: 1.7 }}>
      ...
      {!isPro && transactions.length > 0 && (
        <div style={{ background: "#f59e0b18", border: "1px solid #f59e0b44", borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#f59e0b" }}>Free tier: {visibleTransactions.length}/{FREE_MAX_TX} transactions · 2 weeks</div>
            <div style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>Pro unlocks 100 transactions over 3 months</div>
          </div>
          <button onClick={onShowPro} style={{ fontSize: 11, fontWeight: 700, color: "#f59e0b", background: "#f59e0b22", border: "1px solid #f59e0b44", borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>Go Pro</button>
        </div>
      )}
...
function DebtPayoffSim({ user, onSave, onShowPro }) {
  const [scenarioName, setScenarioName] = useState("");
  ...
  const handleSave = () => {
    if (!user) return alert("Create an account to save scenarios!");
    const label = scenarioName.trim() ? scenarioName.trim() : `Debt Payoff — ${fmtMonths(currentResult.payoffMonths)} to freedom`;
    onSave({
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {validDebts.length > 0 && (
        <Card>
          ...
          <div style={{ marginTop: 10 }}>
            <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>Scenario Name (optional)</div>
            <input value={scenarioName} onChange={e => setScenarioName(e.target.value)} placeholder="e.g. Credit Card Payoff, Debt Free 2026"
              style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: C.cardAlt, color: C.text, fontSize: 13, fontFamily: "inherit", boxSizing: "border-box", outline: "none", marginBottom: 8 }} />
          </div>
          ...
          {r.admin_reply && (
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: `linear-gradient(135deg, ${C.primary}, #1d4ed8)`, display: "grid", placeItems: "center", color: "#fff", fontSize: 12, fontWeight: 900, flexShrink: 0 }}>R</div>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, marginBottom: 3 }}>Reality Estimator</div>
                  <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6 }}>{r.admin_reply}</div>
                </div>
              </div>
            </div>
          )}
...
function AdminPanel({ onClose, user, showToast }) {
  ...
  const saveReply = async (id) => {
    ...
    if (!text) return;
    await sb.from("reviews").update({ admin_reply: text }).eq("id", id);
    setReplyingTo(null);
    loadReviews();
  };
                ...
                <div key={r.id} style={{ background: C.card, border: `1px solid ${r.is_approved ? C.greenBorder : C.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {r.admin_reply && (
                        <div style={{ marginTop: 8, padding: "8px 10px", background: `${C.primary}12`, borderRadius: 8, borderLeft: `3px solid ${C.primary}` }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: C.primary, marginBottom: 2 }}>Your reply</div>
                          <div style={{ fontSize: 12, color: C.muted }}>{r.admin_reply}</div>
                        </div>
                      )}
                    ...
                    <div style={{ display: "flex", gap: 6, marginLeft: 12, flexShrink: 0 }}>
                      <button onClick={() => setReplyingTo(replyingTo === r.id ? null : r.id)}
                        style={{ fontSize: 11, padding: "5px 12px", borderRadius: 6, border: `1px solid ${C.border}`, background: replyingTo === r.id ? C.primary : C.cardAlt, color: replyingTo === r.id ? "#fff" : C.muted, cursor: "pointer", fontWeight: 700 }}>
                  ...
                  {replyingTo === r.id && (
                    <div style={{ marginTop: 12 }}>
                      <textarea
                        ...
                        placeholder="e.g. Thank you for the feedback! We appreciate it 🙏"
                        rows={2}
                        style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.primary}`, borderRadius: 10, fontSize: 13, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box", background: C.cardAlt, color: C.text }} />
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <Btn onClick={() => saveReply(r.id)} style={{ flex: 1, padding: "8px", fontSize: 12 }}>✓ Post Reply</Btn>
                        <button onClick={() => setReplyingTo(null)} style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: "none", color: C.muted, cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
...
export default function RealityEstimator() {
  ...
  const [showEmail, setShowEmail] = useState(() => {
    try { const v = localStorage.getItem("re_show_email"); return v === null ? true : v === "true"; } catch { return true; }
  });
  const toggleShowEmail = () => setShowEmail(v => {
    const next = !v;
    try { localStorage.setItem("re_show_email", String(next)); } catch {}
    return next;
  });
  ...
  useEffect(() => {
    ...
    if (params.get("pro") === "success") {
      ...
      // Re-fetch profile to get updated is_pro status
      SB.getSession().then(async (session) => {
        if (session?.user) {
          const profile = await SB.getProfile(session.user.id);
          const u = { id: session.user.id, name: profile?.name || session.user.email.split("@")[0], email: session.user.email, zip: profile?.zip || "", is_admin: profile?.is_admin || false, is_pro: profile?.is_pro || false };
  ...
  const handleSave = (scenario) => {
    if (!user?.is_pro && DB.getScenarios().length >= 2) {
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "ui-sans-serif, system-ui, -apple-system, 'Segoe UI', sans-serif", color: C.text, paddingBottom: 80, transition: "background 0.2s, color 0.2s" }}>
      ...
      <div className="re-layout" style={{ display: "block" }}>
        ...
        <div className="re-sidebar" style={{ display: "none", background: C.card, borderRight: `1px solid ${C.border}` }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {user.is_pro && <span style={{ fontSize: 9, fontWeight: 900, color: "#0f172a", background: "linear-gradient(135deg, #f59e0b, #d97706)", borderRadius: 4, padding: "2px 5px", letterSpacing: "0.04em", flexShrink: 0 }}>PRO</span>}
                    </div>
                  </div>
                </div>
          ...
          {!user?.is_pro && (
            <button onClick={() => setShowProModal(true)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, border: "1px solid #f59e0b44", cursor: "pointer", fontFamily: "inherit", marginBottom: 4, marginTop: 8, background: "linear-gradient(135deg, #f59e0b18, #f59e0b08)", color: "#f59e0b", fontWeight: 700, fontSize: 14, textAlign: "left", transition: "all 0.15s" }}>
        ...
        <div className="re-content" style={{ maxWidth: 520, margin: "0 auto", padding: "18px 16px" }}>
          ...
          {page === "simulate" && <SimulatePage defaultTab={simTab} user={user} onSave={handleSave} onShowPro={() => setShowProModal(true)} />}
          {page === "dashboard" && <Dashboard user={user} onLogout={handleLogout} onShowAuth={() => setShowAuth(true)} showEmail={showEmail} onToggleEmail={toggleShowEmail} onShowPro={() => setShowProModal(true)} />}
          {page === "compare" && <ComparePage user={user} />}
          {page === "spending" && <SpendingTracker user={user} onShowPro={() => setShowProModal(true)} />}
      ...
      {showProModal && <ProModal onClose={() => setShowProModal(false)} onUpgrade={() => { setShowProModal(false); startCheckout(user); }} />}
      ...
      {showSettings && user && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: C.card, borderRadius: 20, padding: 24, width: "100%", maxWidth: 500, maxHeight: "85vh", overflowY: "auto", boxShadow: C.shadow }}>
            ...
            showEmail={showEmail} onToggleEmail={toggleShowEmail} />
          </div>
        </div>
      )}
    </div>
